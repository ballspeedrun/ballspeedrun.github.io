...
namespace project_1 {
	...
	void print() {
		...
		for (int i = -14; i <= 14; ++i) for (int j = -14; j <= 14; ++j) {
			if (get_g(X + i, Y + j) == print_g[i + 14][j + 14]) continue;
			jump((j + 14) * 2, (i + 14));
			print_obj(get_g(X + i, Y + j));
			print_g[i + 14][j + 14] = get_g(X + i, Y + j);
		}
	}
	...
	void init(int x, int y) {
		place(x, y);
		cnt_up = aut_up = lst = falin = bouncin = fall_cnt = 0;
	...
	void free() {
		...
		else if (!aut_up and T - tfall > speed) {
			tfall = T;
			if (!falin) {
				falin = true;
				if (!cnt_up) cnt_up += 1;
